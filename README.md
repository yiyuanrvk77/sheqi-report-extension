...
识别走 **PaddleOCR 在线 API（PP-OCRv6）**，token 通过**环境变量 `PADDLEOCR_ACCESS_TOKEN`** 提供，**不落文件、不入 git**。

1. 在 [AI Studio PaddleOCR](https://aistudio.baidu.com/paddleocr) 申请 token；
2. 设置环境变量 `PADDLEOCR_ACCESS_TOKEN=<你的 token>`；
3. 未配置或识别失败时，自动转人工输入。
